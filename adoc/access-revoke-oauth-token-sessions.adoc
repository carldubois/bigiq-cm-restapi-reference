...
|Type|Name|Description|Schema|Default
|*Path*|*objectId* +
_required_|Unique id refering to a token.|string(UUID)|
|===
...
|*generation* +
_optional_ +
_read-only_|A integer that will track change made to revoke-oauth-tokens task collection object. generation.|integer(int64)
|*items* +
