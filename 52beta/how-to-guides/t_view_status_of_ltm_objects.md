...
You can retrieve a list of available kinds for LTM, Network, and System objects. A typical post looks like this:
```
...
You can retrieve a listing of the current configuration values for a virtual serverincluding it's selfLink), by posting a query to it's IP address. A typical post looks like this:
```
...
To view the current availability status of a virtual server, simply add '/stats' to the end of the virtual server's selfLink and do a GET on that URL.
...
There are a couple of mechanisms for locating that nodes that are used by a virtual server.
...
#### 1. Using the referenceKind URL Parameter
For this method, use the referenceMethod* 'resourceReferencesKind', to specify the virtual server as the *referenceLink* and the node kind as the *referenceKind*. A typical post looks like this:
```
...
#### 2. Manually walking each reference
For this method, post a Get to the virtual server's *poolReference* A typical post looks like this:
```
...
To get a pool's member reference subcollection, you can post a query to the members. A typical post looks like this:
```
...
To get a list of a virtual server's node references, you can post a query to the members. A typical post looks like this:
```
...
To get a member's node reference, you can post a query to each member. A typical post looks like this:
```
